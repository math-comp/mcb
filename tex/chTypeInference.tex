...
The objective of the so called \C{bigop} library is to provide
...
as a list, the general expression like $f(i)$ by a function \C{(fun i =>
...)}, while the operation of evaluating a function on all the
elements of list and combining the results is the \C{fold} operator.
...
generically, since the filtering condition is not explicit.  For
example such equation clearly holds for any filtering, something hard
to express if the filter is mixed with the general expression.
$$
\sum_{i < 2n,\,  2 \nmid i} i \;+ \sum_{i < 2n,\,  2 | i}  i = \sum_{i < 2n}
$$

At the light of that, our formal statement will be
...
Knowing that we target the \C{foldr} generic iterator, we need to
obtain something like

\begin{coq}{name=bigop2}{}
Definition sum_odd_def n :=
  foldr
    (fun acc i => if odd i then i + acc else acc)
    0 (index_iota 0 n.*2)
\end{coq}

The notation can be split into two components: 
on one hand the general expression, the filter, and the iterate
operation (line 3); on the other hand the generic iterator, the initial
value and the range (lines 2 and 4).  We start from the former.

For notational purposes it is important to identify the various
components, that in the \C{sum\_odd\_def} definition above are mixed.
...
Where \C{op} is the iterated operation, \C{idx} the neutral element
\C{r} the range, \C{P} the filter (hence the boolean scope)
and \C{F} the general expression.

\begin{coq}{name=bigop21}{}
Lemma sum_odd n : \big[addn/0]_(i <- index_iota 0 n.*2 | odd i) i = n^2.
\end{coq}

To obtain a notation closer to the mathematical one we can specify
...
Such notation is placed in \C{nat_scope} and is indeed specialized
on \C{addn} and \C{0}.  Finally we specialize the notation to
a numerical range:
...
inside the \C{nat\_scope}.  The proof of the general theorem is left as
...
The \C{bigop} constant is ``locked'' to make the notation steady.
To unravel its computational behavior one
has to rewrite with \C{unlock}.
...
operation.
For example the lemma \C{big\_nat\_recr} expresses that one can pull
...
\C{idx} and any generic expression \C{F}, while the filter \C{P} is
fixed to \C{true} (i.e. no filter).
Hence to apply it to our goual one needs to temporarily move the
finter condition inside the body \C{F} using the \C{big\_mkcond}
lemma.
...
Now the lemma \C{big\_nat\_recr} applies, and pulls out 5 while
shrinking the range.
...
It is clear that for the two expressions (with and without \C{big\_nat\_recr})
to be equal one needs to associativiy of \C{addn} (and also that \C{0} is
neutral).
In other words the lemmas requires the operation \C{*\%M} to form
a monoid together with \C{idx}.

We detail how this requirement is stated, and automatically satisfied by
Coq in the case of \C{addn}, in the next subsection.
...
symbols to state that, for example, \C{addn} together with \C{0}
forms a monoid.
...
The first lemma, \C{big1\_eq}, states that iteraring the operation to
...
Help to read the statements, where does one see that it is a monoid.
...
rewrite -[odd _.+1]/(~~ (odd _)) odd_double /= addn0 {}IHn.
by rewrite -addnn -!mulnn !mulSn !addSn !mulnS !addnS addnAC !addnA.
Qed.
\end{coq}
...
Maybe also for the lambda user show that \C{[the Foo of X]}.
...
When you write \C{\{set nat\}} the system gets
\C{(set\_of ?T (Phant nat))}.
The unification problem is \C{(phant (Equality.sort ?T))} versus \C{(phant nat)}
and then the definition uses \C{?T} as inferred by the preious
problem to obtain \C{(seq (Equality.sort nat_eqType))}.
