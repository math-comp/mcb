...
The rules of the \mcbCIC{}, as the ones sketched in~\ref{sec:ttch},
are expressed on the syntax on terms and
...
\footnote{The meaning of a symbol in math is even deeper: by
	writing $a * b$ one may expect the reader to figure out which
	ring she talks about, recall its theory, and use this
	knowledge to justify some steps a proof.  By programming type
	inference we model this practice in~\ref{rec:itf}.}
...
: T, P)}.  Indeed \C{T} is not a type, only its \C{sort} projection is.
...
declaration typically look very confusing: ``\ldots has type \C{nat}
...
by better specifying the type that parameterizes a generic container
...
\mcbLEVEL{1}
\mcbsection{The generic theory of ``big'' operators}\label{sec:bigop}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
As already mentioned in \ref{sec:bigopnat}, 
...
elements of list and combining the results by the \C{foldr} iterator.
...
\lib{bigop} file in order get acquainted with the naming policy used
in that library.
...
\mcbPROVIDE{Phantom}
\mcbLEVEL{3}
\mcbsection{Querying canonical structures}\label{sec:phantom}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
