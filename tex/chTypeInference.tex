...
the interface.  An instance is an example of an algebraic structure,
an object that fits an interface.  The programs we will write to
extend type inference typically define the link between existing
...
\begin{coq}{name=parsecs2}{title=Example,width=5.3cm}
Check (3, true) == (4, false).
\end{coq}
\coqrun{name=cs2}{ssr,eqtype,eqn,bety,infix,declcs,paircs,declcs2,parsecs2}
\begin{coqout}{run=cs2}{title=Response,width=6.7cm}
(3, true) == (4, false) : bool
\end{coqout}

Behind the scenese

\begin{coq}{name=parsecs3}{title=Example,width=5.3cm}
Check
@eq_op
...
is compatible with equality: two ground terms compare to equal if and
...
a comparison function compatible with Leibnitz equality (substitution)}
...
drives the inference of canonical structure inference. Following
...
Hence the real, and final, definition of \C{eqType} is.
...
In this simple case there is only one, named, axiom.
Said that, nothing really changes: the \C{eqType} structure 
relates a type (called \C{sort}) with a signature (called \C{class}).
Also remark that the use of \C{:>} instead of \C{:} to type the
field called \C{sort}.  This tells Coq to declare the sort projection
as a coercion, enabling one to write \C{(forall T : eqType, forall x y
: T, P)}.  Ineed \C{T} is not a type, its \C{sort} projection is.
...
\section{Using and building a generic theory}
...
proof, by means of the \C{eqP} lemma, uses the axiom of T1 and T2.
...
So now, when one write 

\begin{coq}{name=eqtype}{title=eqType}
(3, true) == (4, false)
\end{coq}

Coq infers both a comparison function and a proof that

\begin{coq}{name=eqtype}{title=eqType}
reflect ((3, true) = (4, false)) ((3, true) == (4, false))
\end{coq}

Such property can be used, again by using eqP, on an arbitrary eqType.
...
Qed.
\end{coq}
\coqrun{name=uniqp}{ssr,uniq,undupuniq,abort}
...
Maybe this is too much for here.

\begin{coq}{name=seqeqtype}{}
Section EqSeq.
...
To complete the tour: we have an interface \C{eqType}, base instances
for \C{nat} and \C{bool}, ways to combine instances (pair and seq),
abstract theories on the \C{eqType} interface.

The last bit is applying such theories to an example:
