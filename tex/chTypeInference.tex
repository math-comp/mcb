...
The objective of the \C{bigop} library is to provide compact notations 
...
...)}. Finally the operation of evaluating a function on all the
elements of list and combining the results is the \C{foldr} operator.
...
to the term \C{(fun acc i => if odd i then i + acc else acc)}
\item
the generic iterator, the initial value and the range
corresponding to \C{foldr}, \C{0} and \C{(index\_iota 0 n.*2)}
\end{itemize}

We start from the former.
For notational purposes it is important to identify the various
sub components, that in the \C{(fun acc i => ...)} expression
are mixed.  For that purpose we craft a box, \C{BigBody}, with
...
one needs to associativiy of \C{addn} and that \C{0} is
neutral.
