...
\footnote{The meaning of a symbol in mathematics is even deeper: by
	...
	knowledge to justify some steps of a proof.  By programming type
	inference appropriately we model this practice in~\ref{rec:itf}.}
...
The \C{Equality} module enclosing the record acts as name space:
\C{type}, \C{sort},
...
: T, P)}.  Indeed \C{T} is not a type, only \C{(sort T)} is.
...
by better specifying the types that parameterize a generic container
...
constants that should not be automatically simplifies away, unless
they
