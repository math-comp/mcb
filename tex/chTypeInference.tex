...
to \C{(fun acc i => if odd i then i + acc else acc)}
\item
...
Note that in this case the index, i.e. the name of the bound variable,
is identical to the general term, but this is not true in general.
Indeed the first component of the \C{BigBody} box is there only to
help the notation system to figure out the name of the bound variable.
...
satisfy in order aceess a class of generic lemmas.
Chapter~\ref{..} adds other interfaces to the picture and organizes
...
One cannot search with \C{\\sum}, since all lemmas are about
monoidal operation, and not addn.

Tricks of the trade are:
\begin{itemize}
\item lemma names contain ``big''
\item ...
\end{itemize}
