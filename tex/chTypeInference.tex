...
The notation standing for \C{foldr} can be split into two components: 
on one hand the general expression, the filter, and the iterate
operation \C{(fun acc i => if odd i then i + acc else acc)};
on the other hand the generic iterator, the initial
value \C{0} and the range \C{(index\_iota 0 n.*2)}.
