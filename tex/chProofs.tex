...
\begin{coqdef}{name=addnA}
Lemma addnA (n m k : nat) :
  n + (m + k) = m + n + k.
Proof.
\end{coqdef}
\begin{coq}{def=addnA}{width=6cm,title=Stating quantified lemmas}
Lemma addnA (n m k : nat) :
  n + (m + k) = m + n + k.
Proof.
\end{coq}
...
\begin{coq}{name=coqercion}{title=Coercion}
Lemma leq0n n : 0 <= n.
\end{coq}
...
\begin{coq}{name=implication}{title=Implicative sentences}
