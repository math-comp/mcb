...
the semicolon. The proof script (up to the end of the proof of the first
goal) then looks like this:
% [DG] The preceding paragraph creates the impression that the
% semicolon is just a synonym for a period. Not true, and probably
% worth explaining.
...
% [DG] Looks weird to summon the list type, after having worked with
% seq all the time.
...
make use of the \cite{apply:} tactic, whereas the \C{elim:} tactic
...
second subgoal that follows \C{rewrite dvdn_addr} a side condition
