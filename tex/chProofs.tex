...
\mcbsection{Simple formal statements}
...
do not feature parameter variables. Just like we used the \C{Check}
...
\marginnote{Ensure we've seen \C{Check} in chapter 1}
...
obtained by applying the constant \C{eq} to two arguments, of the same
type, which results in a well-formed term of type \C{Prop},
for \emph{proposition}. Throughout this book, we will use the word
\emph{proposition} for a term of type \C{Prop}, something one
typically want to prove. The \C{About} vernacular
...
\marginnote{Ensure we've seen \C{About} in chapter 1, with this
  policy}
...
of the handsides of the equality we state: \C{3 = 3} unfolds to
\C{eq _ 3 3} and the missing value is set at \C{nat}, the type of
\C{3}. Similarily, \C{true = false || true} unfolds to
\C{eq _ true (false || true)} and the missing value is set at
\C{bool}, the common type of \C{true} and \C{false || true}.
...
the \C{Lemma} keyword, followed by the name we chose for the lemma and
the lemma statement. \marginnote{What about the synonyms?}
The \C{Proof} keywords marks the beginning of the proof and the system
...
\marginnote{Is is an ok transition? \C{Lemma} is needed to postpone forall...}
For now, let us just admit this result, using the \C{Admited}
command.
...
\marginnote{I guess it is too early to say that this is a type...}
...
\C{Definition} syntax we used in Chapter~\ref{ch:prog}, which also
applies to the \C{Lemma} keyword (and its siblings):

\marginnote{Here and may be in the next examples, point forward to the
later section which discusses names.}
...
\marginnote{The previous chapter should make comments on the
  possibility to gather variables of the same type. Here I cast them
  for the sake of pedagogy.Also I am cheating here and I do not use the associative
  predicator... And the display here is related to the tunning of
  notations, shall we say a word about it at this stage?}

For the next example, our first example of ground
boolean equality is an instance of the identity \C{orbT}:
...
\C{orbT} statement we saw at section~\ref{ssec:id} is a special case
...
case of \C{orbT}, the truth table of $b \wedge \top$ is the same as
...
type.\marginnote{Make sure  ``boolean predicate'' is introduced in the
  previous chapter.} For instance, we can state that for any term
\C{n : nat}, the following comparison holds:
...
its truth value is \C{true}, whatever term of type \C{nat} is
substituted for the parameter \C{n} in the statement.

We can also express that boolean statements are equivalent, like we
did in the case of \C{orbA}:
...
\mcbsection{Simple formal proofs}
...
\mcbsection{Universal statements}
...
\mcbsection{First examples}\label{ssec:firstex}
...
its name.  The name \C{my_first_lemma} we chose in
