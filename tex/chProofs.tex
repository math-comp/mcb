...
From now on we will omit the \C{Admitted} proof terminator, and
simply list the statement of the lemmas to discuss their formulation.
...
expresses a conjunction of hypotheses:
...
Now that \C{b} is a concrete value each proof amounts to a
...
Once the first goal is solve we have only one subgoal left, which we solve
...
\C{by} command, which concludes the proof by reflexively.
...
that are instances of these predicates. For instance a common suffix \C{C} is
...
\end{coqout}
\C{commutative} is a polymorphic function taking a binary operation and
