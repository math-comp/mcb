...
Section~\ref{ssec:ind}. We resort to an auxiliary lemma, called
\C{muln0}:
...
(m.+1 == 0) || (0 == 0)
\end{coqout}
which preforms the desired replacement. Note that we did not provide
the value of the parameter \C{n} used in the statement of \C{muln0}:
the single possible occurrence of a subterm matching the left-hand
\marginnote{Put the pattern, and use this word.}
side of the identity has been found by the matching algorithm of the
\C{rewrite} tactic. We can now conclude the proof script, using the
prenex \C{by} tactical:

\marginnote{Where do we introduce the \C{//} intro pattern?}
