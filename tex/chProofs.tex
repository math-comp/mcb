...
commands. In the course of this book, we will see that chosing the
...
whenever possible, an approach that will be developped fully in
...
Let us start with calls to the \C{Check} command, which we have used
...
results in a swell-formed term of type \C{Prop}, for \emph{proposition}.
...
\marginnote{I use Locate here because the output of Search is much
  more verbose.}
...
\C{(false && true = true)} unfolds to \C{(eq _ (false && true) false)}
...
list of named hypotheses forms the current contexte and is printed
...
Boolean identities play a central role in the \mbcMC{} library. They
...
predicate is thus a ``test''  and any such test can be used to form a
proposition by equating its result to \C{true}.
More generally, boolean identities involve arbitrary boolean
predicates and boolean connectives. They may also feature variables of
