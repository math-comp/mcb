...
\begin{coq}{width=4cm,title=Ill-typed Equality}
Check 3 = [:: 3].
$~$
\end{coq}
...
the lemma statement. \marginnote{What about the synonyms (theorem,
  remark, corollary,...)?}
...
Note that \C{Lemma}, like \C{Definition}, allows for dropping type
...
instance, the~\C{orbT} statement:

\begin{coq}{title=A boolean identity}
...
previous chapter.} For instance, we can state (and very soon prove)
...
to discuss this but we should do it somewhere.}
...
are the same, and the tactic \C{by []} is the command that
implements this nature of proof step. \marginnote{I chose not to
  expose reflexivity because we never use it but shall we mention this
for users of plain Coq? Or use done? Finally, \C{by []} is more
natural for the last examples of the section. Also where comes the
...
\C{true}. \marginnote{Should these be exercises? should we add failure
  examples?}
...
And now we can wrap up explaining \C{by}. And show a second example on
\C{nat}.
