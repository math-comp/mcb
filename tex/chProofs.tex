...
argument, \C{(m.+1 * 0)} does not evaluate symbolically to \C{0}.
...
The first step is performed using the following equation:\marginpar{It is not true anymore that we defined subtraction in chapter 1}
...
chapter~\ref{ch:prog}:\marginpar{it is not true anymore that we defined
subtraction in chapter 1}
...
parameters all feature prenex universal quantifiers:\marginpar{Is the
word {\em prenex} appropriate here?}
...
\index[ssr]{\C{apply:}}
The work performed by the \C{apply} tactic is up to computation:
...
the SSReflect language user manual~\cite{ssrman}.\marginpar{I don't like ``for the casual reader''}
