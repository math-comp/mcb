...
Lemma foldr_cat T R f (s1 s2 : seq T) (z0 : R) :
...
% Qed.
%
% Lemma foldr_cat T R (f : T -> R -> R) (s1 s2 : seq T) z0 :
%   foldr f z0 (s1 ++ s2) = foldr f (foldr f z0 s2) s1.
% Proof.
%   move: s2 z0.
%   elim: s1 => [| p t1 IH] //=.
...
The same result could have been obtained using \C{rewrite \{2\}/leq},
because \C{(n1 <= n2)} is the \emph{second} appearance of the \C{leq}
constant in the goal. The \C{\{k\}} prefix (for a positive integer \C{k})
generally has the effect that the tactic is being applied in the
\(k\)-th of the possible positions where it is applicable; for
instance, in the proof of \C{silly_example}, we could have used
\C{rewrite \{2\}addn0} instead of \C{rewrite [in RHS]addn0}.
