...
corpus of definitions and properties. For instance, the following prelude:
...
End Chinese.
\end{coq}
corresponds to a mathematical sentence of the form: \emph{Let $m_1$
  and $m_2$ be two coprime natural numbers\dots}.
\marginnote{We need to say later that outside the scope of the
  section, the hypothesis is added to the statement of lemmas whose
  proof use it. But it's too early here since we would need to say
  also that m1 and m2 are universally quantified.}
...
\C{by} typically prefixes another tactic (or a list of tactics): it is
...
the second. The proof of \C{leq0n} thus goes by case analysis on
 term \C{(n : nat)}, as it appears as a first argument to these
...
\C{k} which pops up in the second branch as the argument of the \C{S}
...
both hand sides of the equality evaluates to \C{false}.  The final
...
evaluates to \C{false}: by the definition of \C{muln} the term
...
Back yo our example, we use the \C{rewrite} tactic with lemma
...
(m * n1 - m * n2 == 0) =
  (m == 0) || (n1 <= n2)
\end{coqout}
Unfortunately, the command \C{rewrite leqE} only affects the first
...
\marginnote{The negation in the conclusion is a bit weird. Shall we
  remove it?}
...
contrapositive. For this purpose, we use the following lemma:

\begin{coq}{}{}
Lemma contraL (c b : bool) : (c -> ~~ b) -> (b -> ~~ c).
\end{coq}
\index[coq]{\C{contraL}}
which describes the shape of the contrapositive of statements with an
explicit negation in their conclusion and thus describe a family of
instances of the contraposition law. The \C{apply: contraL} tactic
finds the appropriate values of the premisse and conclusion and thus
instantiate the scheme, leaving us to prove that \C{p} is not a
divisor of \D{(m `! + 1)} under the assumption that \C{p} is smaller
then \C{m}:
...
\C{c, b} of lemma \C{contraL} are \C{(p <= m)} and
...
premisse \C{(c -> ~~ b)} of lemma \C{contraL}.

The next step in our formal proof is to give a name to the hypothesis
\C{(p <= m)}.
...
functionalities, while \C{apply:} does not. In particular, the
\C{elim} tactic has special skills that allow guessing the value of
the parameter \C{P} of the induction scheme, which the \C{apply:}
tactic doesn't have.
...
\emph{abstracting} the term \C{n} in the goal (see
...
\marginnote{We do not know foldl.}
...
\marginnote{Why is this section here (and not at the end for ex.)?
  Plus, I have removed the list of suffixes from the manual.}
...
\marginnote{This is not true anymore. Should this list be part of the book?}
The naming convention is documented in the reference
manual of Ssreflect~\cite[Section 11]{ssrman}.
Let's get a flavor of it by examples:
...
short names, so that they can be easily typed in and at the same
...
term \C{(m * n1 <= m * n2)} is first ruled out because the first argument of
