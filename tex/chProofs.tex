...
\begin{coq}{def=leqn0,done}{width=6cm,title=Negative natural number}
Lemma |*leqn0*| n : (n <= 0) = (n == 0).
\end{coq}
...
\begin{coq}{def=leqn02,done}{width=6cm,title=Negative natural numbers}
Lemma |*leqn0*| n : (n <= 0) = (n == 0).
Proof.
case: n => [| k].
  by [].
...
\begin{coq}{def=almostMulnEq2}{title=First case analysis,width=7cm}
Lemma |*muln_eq0*| m n :
  (m * n == 0) = (m == 0) || (n == 0).
Proof.
case: m => [|m].
...
case: n => [|k]; last first.
\end{coq}
\begin{coqout}{def=almostMulnEq2}{title=First case analysis,width=6cm}
...
case: n => [|k]; last first.
  by [].
\end{coq}
\begin{coqout}{def=almostMulnEq3}{title=First case analysis,width=6cm}
1 subgoals
...
more than symbolic computation to show that the the left hand side is
\C{true} as well, as the fact that \C{0} is a left zero for
multiplication requires reasoning by induction on the code of the
multiplication. The next sections illustrate in particular how to
complete this proof.

Before moving on we introduce an optional part of an intruction pattern
that tells the system to try to solve all goal by mean of \C{by[]}.
...
Observe that the \C{//} intro pattern has been tried on both goals:
it solved only the first one so we are left with the second one.
Similarly, we can get rid of the second goal produced by the case
analysis on \C{n}.

\begin{coq}{def=almostMulnEq31}{title=Some other lysis,width=7cm}
Lemma |*muln_eq0*| m n :
  (m * n == 0) = (m == 0) || (n == 0).
Proof.
case: m => [|m] //.
...
We will see that \C{//} (is like a speech filler) can be placed 
in the middle of other combined tactics, and not only in introduction
pattern.
...
In this section, we explain how to use known identities, ones that have
for instance already been proved formally,
in order to replace subterms of a goal with other terms during the
course of a formal proof. In other words, we explain how to perform a
\emph{rewrite} proof step, thanks to the eponym \C{rewrite} tactic.

Let us come back to the proof that we left unfished at the end of the
previous section:
...
which is equal
modulo computation to \C{true = true}. But because of the definition
of \C{muln} by case analysis on its \emph{first} argument,
\C{m.+1 * 0} does not evaluate symbolically to \C{0}: this equality
holds but requires a proof by induction, as explained in
...
\marginnote{But this is not the way the lemma is stated in the library...}

\begin{coq}{def=muln0}{title=Multiplying by zero on the left}
Lemma |*muln0*| n : n * 0 = 0.
\end{coq}

and we call the \C{rewrite} tactic:
...
rewrite muln0.
\end{coq}
\begin{coqout}{def=almostMulnEq4}{title=First case analysis,width=6cm}
1 subgoals
...
(m.+1 == 0) || (0 == 0)
\end{coqout}
which preforms the desired replacement.

The \C{rewrite} tactic takes in input an equation and replaces
instancea of its LHR with its corresponding RHS and vice versa.
For example the LHS of muln0 can be read as a \emph{pattern}
\C{(_ * 0)} where we use \_ to denote a wildcard. Indeed the
identity is valid for any value of its parameter n.
In this particular case the RHS does not depend on the parameter n.
The tactic find automatically where the replacement takes place by
searching the goal for a subterm matching the pattern \C{(_ * 0)}.
There is only one such subterm, namely \C{(m.+1 * 0)}, that is
hence the replaced by \C{0}, the RHS of \C{muln0}.
\marginnote{Patterns are also needed by Search and are a quite
pervasive concept.  Maybe hint that here.}


We can now conclude the proof script, using the
...
Now let us consider the case of lemma \C{|*leq_mul2l*|}, which
provides a necessary and sufficient condition for the comparison
\C{m * n1 <= m * n2} to hold:

\begin{coq}{def=LeqMul2l}{title=}
Lemma |*leq_mul2l*| m n1 n2 : (m * n1 <= m * n2) = (m == 0) || (n1 <= n2).
\end{coq}

The proof uses the following three ingredients: the definition of leq,
the distributivity of multiplication over subtraction and finally
the lemma we just proved \C{muln_eq0}.

The first step is to unfold the definition of leq to unravel subtraction.
This can be performed using the rewrite tactic and the following syntax.

\begin{coq}{def=LeqMul2lproof}{title=}
rewrite /leq.
\end{coq}
\begin{coqout}{def=LeqMul2lx}{title=First case analysis,width=6cm}
1 subgoals
...
Note the statement of \C{mulnBr}.  We use it right to left hence the - sign.
Finally we use \C{muln_eq0}.
...
The proof can be completed with \C{by[]}.  Subsequent rewriting steps can be
combined into a single tactic invocation.
...
RHS of \C{mulnBr} to factor out m.  Such step being just computation has no
logical content, but helps driving the following rewrite.
By looking at the RHS of \C{mulnBr} rewrite infers the pattern
...
\C{in RHS, [_ + a]addnC} and maybe \C{[_ + _]mulnC}.

Explain: first find the first instance of the pattern, then replace all
occurrences.  As a last tuning, \C{{2}}.

Take a goal with many wrong parentheses and normalize it? Or many \C{+0}.
Explain \C{!}.
...
Do other examples, with an implication.
...
\C{(negbK true)} useful for instance in the case of \C{(addnC a)}. The
\C{apply} tactic does the job of finding the right instance from the
goal.
...
is what \C{move=>} does (same as in \C{case}, but no branch).
