...
predicates, the MC library uses a systematic naming policy for the lemmas
...
We see it in action IN FIXME INFINITY OF PRIMES.

The action performed by the command \C{apply: contraL} is
the following one:

\begin{coqout}{}{width=6cm,title=Before}
m, p : nat
prime_p : prime p
====================
p %| m `! + 1 -> ~~ (p <= m)
\end{coqout}
\begin{coqout}{}{width=6cm,title=after}
m, p : nat
prime_p : prime p
====================
 p <= m -> ~~ (p %| m `! + 1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\mcbLEVEL{3}
\mcbsection{Rewrite, second dan}\label{sec:rewrite2}
