...
all goals zero-or-more times.  For example, the side condition
\D{p \%| m`!} spawned by \C{rewrite dvdn_addr} in our proof of
\C{example m p} can be solved right away with an application of this
prefix.

Let us illustrate this on a slight variation of the above example
\C{Lemma example m p} (the difference being that the conclusion
\C{m < p} has now been rewritten as \C{\~\~ (p <= m)}):

\begin{coq}{}{}
Lemma example2 m p : prime p -> p %| m `! + 1 -> ~~ (p <= m).
Proof.
