...
theorems are stated and proved, focusing on simple statements and
basic proof commands.  

Stating a lemma is a very delicate matter.  Even if two formulations for the
same statements are logically equivalent, one can be much simpler to prove or
invoke in the proof of another theorem.  The approach we follow in the current
chapter and that we fully develop in chapter~\ref{ch:boolrefl} is paradigmatic
in the \mcbMC{} library and roughly consist in favouring the use of computable
...
parameter variables. For instance $2 + 2 = 4$ is a ground
  statement, but $(a + b)^2 = a^2 + 2ab + b^2$ is not.
...
type \C{Prop}, for \emph{proposition}. 
...
command, which lets one obtain information on a constant, like its
...
function letting build a proposition.  Sometimes we say
\emph{boolean predicate} to indicate a function to \C{bool},
i.e. a test one can use to form a proposition by equating its
result to \C{true}, as in section~\ref{sec:bstatements}.
...
range over types
  This is why we were able to use the same equality
...
user%\footnote{The system even raises an error if it is provided.} 
but
rather guessed from the type of the hand-sides of the equality we state: \C{(3
= 3)} unfolds to \C{(eq _ 3 3)} and the missing value is set to
\C{nat}, the type of \C{3}. Similarly, \C{(true = false || true)}
unfolds to \C{(eq _ true (false || true))} and the missing value is found
to be \C{bool}, the common type of \C{true} and \C{(false || true)}.
...
here. The \C{Proof} keyword marks the beginning of the proof text and
...
From now on we will omit the \C{Admitted} proof terminator, and simply list the
statement of the lemmas to discuss their formulation.  Indeed we the rest of
the present section is dedicated to a tour of more complex formal statements.
...
for the boolean disjunction operation \C{orb}. Recall that
...
two propositional formulae are equivalent, or that \C{(b || true)} is a
...
that for any term \C{n : nat}, the following boolean test holds:
...
We can express that two boolean statements are equivalent using
...
We can omit the type of the parameters: they are all of type \C{nat}.
...
expresses a conjunction of conditions:
...
implements this nature of \emph{trivial} proof step.
\C{by} typically prefixes another tactic (or a list of thereof) and 
checks that such tactic trivializes the goal.  In this case
the goal is already trivial, hence the \C{[]}
standing for an empty list of tactics.
...
the statement of \C{my_first_lemma}.  On the contrary
\C{Admitted} happily accepts false statements!
...
1)}, or \C{(odd 5)}, because both expressions \emph{compute} to
\C{true}.
...
also \C{(2 + n = n.+2)}, which requires a longer computation.

The last word to be said about \C{by []} it that it is a terminating
tactic.  If it is unable to solve the goal it fails and stops
\Coq{} from processing the proof script.  For this reason
it is also coloured in red, so that the eye can immediately spot that a 
...
definition of \C{negb}, but also performing a \emph{case analysis}
...
perform a case analysis on \C{b}, whose
...
one in which the parameter \C{b} takes the value \C{false}.


As a consequence, we are going to provide two distinct pieces of
script, one per each subproof to be constructed. 
We start with the branch associated with the \C{true} value, and
to signal it is a sub proof we indent the corresponding script.
Now that \C{b} is a concrete value each proof is trivial
by computation, in the spirit of what we did in
section~\ref{ssec:proofcomp}.
...
Once the first goal is solved we have only one subgoal left, which we solve
...
\C{n} is the successor of \C{k}.
It can also be  solved by computation, as both hand sides
of the equality evaluates to \C{false}.  The final proof
script is hence:
...
A similar example, again of a boolean equivalence, let us introduce
the need of more powerful proof techniques.
We claim that the product of two (natural) numbers is zero if and only
if one of the numbers is zero.  We also recall
the definition of \C{muln}.
...
to use the ``\C{; last first}'' tactic suffix:
...
To conclude, this branch of the case analysis is
...
To conclude the proof we introduce one more proof command,
\C{rewrite}, that lets us appeal to an already existing lemma.
...
This is not a coincidence since the \mcbMC{} library is huge.  Finding the
``right'' lemma is a key ability in writing formal proofs. Section~\ref{sec:search}
is dedicated to this activity.

The \C{rewrite} tactic can use such lemma to perform the desired replacement.
...
\C{rewrite leqE} only affects the first occurrence of \C{<=} and we
...
\index[concept]{rewrite!multiple rules}
...
\C{rewrite}.  Lets now move on to a more interesting class
...
In chapter~\ref{ch:prog}, we have defined functions
with boolean values; they are named 
\emph{boolean predicates}.
 For instance, the \C{leq} predicate is defined as
a boolean predicate on natural numbers, as:
...
Recall the definition of \C{nth} in exercise~\ref{ex:nth}.
...
\index[coq]{\C{rel}} Remark that \C{(rel T)} is an abbreviation for
the type \C{(T -> T -> bool)}.
...
Definition left_id T e op := forall x, op e x = x.
...
\end{coqout}
\C{commutative} is a polymorphic function, in \C{T},
taking a binary operation and
...
Chapter~\ref{sec:ttch} will
provide a more in-depth study of these propositions.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Proving with conditional lemmas}
...
we show that any prime divisor \C{p} of \D{(m `! + 1}) is greater than \C{m},
or better that it is not the case that \C{m} is bigger.
...
prove that \C{p} is not a divisor of \D{(m `! + 1)} under the assumption
...
\C{(p <= m)} and \D{(p \%| m `! + 1)}, by comparing the statement to be
...
Since $p$ is a prime, it cannot be $1$.  
...
As for the type variable in the declaration of the (polymorphic) data type
\C{seq}, the language of \Coq{} gives to $\mathcal{P}$ a regular status, and
induction, instead of being a ``schema'' becomes is a regular lemma.
...
Lets prove by induction on \C{m} that \C{0} is neutral on the
right of \C{addn}.
...
m.+1 + 0 = m.+1
\end{coqout}
We used the \C{//} switch to rule out the base case, since if
\C{m} is \C{0} both sides evaluate to zero.  In order to
replace use the induction hypothesis we pull out of the
sum the \C{.+1} by rewriting with \C{addSn}.
In this proof by induction the value of \C{P} synthesized by
\C{elim:} for us is \C{(fun n : nat => n + 0 = n)}.

Unfortunately proof by induction do not always run so smooth.
To our aid the \C{elim:} tactic tactics provides two additional services.
...
The Ssreflect proof language provides the \C{Search} command to
filter the set of available lemmas providing patterns, like \C{(_ * _ + _)}
or \C{(addn _ _)}, and partial names, like \C{"rev" "cons"}.
...
consistently used to name lemmas.  Indeed the name \C{my_first_lemma} we
...
manual of Ssreflect~\cite[Section 11]{ssrman}.
...
\item In \C{leq0n}, the other symbol involved in the name is ``0'',
that indeed plays an important role in the lemma.  The relative position
of ``0'' and ``n'' is chosen to recall to the user
if 0 occurs the left or the right argument of the binary test \C{leq}.
\item The capital letter suffix ``A'' in \C{addnA} stands for associativity.
\end{itemize}
...
The \C{Search} command accepts as filters partial names.  For example
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\mcbLEVEL{1}
\mcbsection{Rewrite, a swiss army knife}\label{sec:rewrite2}

Approximatively one third of the proof script in the \mcbMC{} library is made of
...
\index[ssr]{\C{rewrite}!\C{//=} (simplify and close trivial goals)}

Last, the unfolding equation \C{leqE} that we used in
...
\C{rewrite} by just prefixing the name of the constant with
\C{/} as in \C{rewrite /leq}.  Indeed \emph{unfolding} a definition is a free
...
typically omitted. However if we
...
\C{(m == 0) || (n1 - n2 == 0)}, itself an instance of the right instance
