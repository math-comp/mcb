...
not available anymore. At any stage of the proof construction, \Coq{}
...
\begin{coq}{name=LeqMul2l}{title=Another example: ordered products}
...
scope of a section the parameters, like \C{m1} and \C{m2}, are
...
\marginnote{dvdnfact has a conjunction in MC}
...
induction hypothesis. We also use the \C{//} switch to rule out the
...
all goals zero-or-more times.  For example, one could consider the
second sub goal that follows \C{rewrite dvdn_addr} a side condition
and solve it on the same line.

Let us illustrate this on an example script that we have already used:
...
The lemma \C{leqE} does not in fact exist in the library. %  Indeed the
...
This indicates in particular that, although the term \C{(m * n1 <= m * n1)}
is equal up to computation to the term \C{(m * n1 - m * n1 == 0)}, the matching
