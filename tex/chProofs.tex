...
\C{true} as well: the fact that \C{0} is a left zero for
...
and we call the \C{rewrite} tactic which preforms the desired replacement:
...
\C{leq} relation, denoted by the \C{<=} infix notation, in order to
...
rewrite rule like \C{mulnBr} or \C{muln_eq0}, was able to identifiy a
subterm in the goal to be substituted -- in theses cases without any
further specification required from the user. The usability of the
...
elects this subterm from the arguments provided to the tactic. Suppose
we execute the command \C{rewrite mulnBr} on the following goal:

\begin{coq}{def=LHS1}{title=Two subterms matching}
Lemma |*foo*| n m p a b c : n * (m - p) = a * (b - c).
\end{coq}

Here both hand-sides are instances of the pattern described by the
left hand side of lemma \C{mulnBr}. The command transforms the goal into:

\begin{coq}{def=LHS1}{title=Two subterms matching,width=7.3cm}
Lemma |*foo*| n m p a : n * (m - p) = m * (n - a) + n * (m - p).
rewrite mulnBr.
\end{coq}
\begin{coqout}{def=LHS1}{title=The first is picked,width=5.2cm}
n, m, p, a : nat
===========================
n * m - n * p = m * (n - a) + (n * m - n * p)
\end{coqout}
Indeed, the matching algorithm traverses the entire goal
left-to-right, looking for the first subterm matching pattern
\C{(_ * _ -_)},  and hence picks subterm \C{(n * m - p)}. Note that
\emph{every} occurrence of this subterm is then rewritten, including
the one located on the right hand-side. Now suppose we want to pick
the other instance of subterm matching this pattern in the goal. We
can use the command \C{rewrite [in RHS]mulnr}, which restricts the
search to the left-hand side of the equality in the goal. The goal
becomes:
\begin{coqout}{def=LHS1}{title=The first is picked,width=5.2cm}
n, m, p, a : nat
===========================
n * (m - p) = m * n - m * a + n * (m - p)
\end{coqout}
because now subterm \C{m * (n - a)} is the first encountered subterm
matching the pattern. Yet this is not so robust so we could rather use
a more precise \emph{pattern} to specify the subterm to be picked by the
matching algorithm, like for instance in \C{rewrite [m * (_ - _)]muln0}.
Now in order to substitute only the right hand side occurrence of
subterm \C{n * (m - p)}, we can use the command
\C{rewrite [X in _ = X + _]mulnBr}, using a pattern variable \C{X},
bound in the pattern \C{(_ = X + _)}. See the reference
manual~\cite{ssrman} for more.
\marginnote{To be  improved.}

\marginnote{Shall we really use as a first example one which requires unfolding?}
END OF REWRITING.
...
\C{forall} quantifier. This universal quantifier binds a boolean, and
...
Quantifiers can also be used to \emph{define} logical statements,
...
which features two parameters \C{f} and \C{g} and one universal
quantification.

\marginnote{
Warning, do not confuse predicates and their types:
\C{About eqfun.
(* eqfun : forall A B : Type, (B -> A) -> (B -> A) -> Prop *)}
But this is too early: requires CH.}

When several definitions and/or lemmas share the same family of
parameters, these can be factored using the \C{Section} mechanism of
\C{Coq}. For instance, the \C{ssrfun} library includes a catalogue of
...
Implicit Types (rT aT : Type) (f : aT -> rT).

Definition |*injective*| rT aT f :=
  forall x1 x2, f x1 = f x2 -> x1 = x2.

Definition |*cancel*| rT aT f g := forall x, g (f x) = x.

Definition |*pcancel*| rT aT f g := forall x, g (f x) = Some x.

(* ... *)
End Injections.
\end{coq}

\marginnote{Were sections already mentionned in chapter 1? Shall we
  use section variables or implicit types?}
Observe the results of \C{About} queries before and after closing the
Section. There is no problem with quantifying on functions in
\C{Coq}'s logic. It is even possible to quantify over predicates, and
...
\C{n.+1 <= m.+1}.\marginnote{Dunno what to say here...}
