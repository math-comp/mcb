...
that can be excuted. As a consequence we borrow from computer science
...
\Coq{} provides a command to retrieve relevant informations about the
...
which confirms the type informations about the domain and codomain of
...
requires first descrbing in more details the formalism underlying the
...
here again informations on the type of the arguments and values of
\C{f2}:
...
subexpressions separated by spaces. The symbol \C{f2} is the deepest
subexpression and it is applied to \C{2} in \C{(f2 2)}. The value of
this subexpression is a function, which can be written:
...
several subexpressions side by side should be read as if there were
...
\C{((repeat_twice f) 2)} and features three subexpressions separated by
spaces. The symbol \C{repeat_twice} is the deepest
subexpression and it is applied to \C{f} in \C{(repeat_twice f)}.
According to the definition of \C{repeat_twice} function, the value of
this subexpression is a function, which is then applied to \C{2}.
% The value of the
...
The process of abstraction described at the beginnig of this section
...
abbreviation for \C{B}. It is specially usefull when \C{B} happens to
...
readable by briging closer the name of the abbreviation \C{x} and the
...
the type and describe the computational behaviour of these operations
