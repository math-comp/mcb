...
formalize the respective arithmetic confer a distinctive status to
...
\marginnote{Refer to~\cite{BC04} for more on termination issues?}
...
the type of its arguments; thus, when defining a function such as
\C{subn}, we could not simply leave \C{subn m n} undefined when
$n > m$.  (At least not without changing the input or the output types
...
Actually, the strange behavior of the \C{subn} function described
above has its virtues:  In particular, it allows defining the
less-than-or-equal relation \C{leq} on naturals as follows:
\begin{coq}{name=nat_leq_def}{}
Definition leq m n := m - n == 0.
\end{coq}
And this is indeed how \C{leq} is defined in \mcbMC{}.
