...
\marginnote{Cite a standard reference?}
In many cases these
...
The two first occurrences of \C{nat} in the response
 \C{nat -> nat -> nat} assert that function \C{f2} has two arguments,
...
Back to our example, here is an alternate definition
 \C{f2'} which has a single argument \C{n} of type \C{nat} and returns
...
Queries on the respective types of \C{f2} and \C{f2'} provide identical answers:
...
the definitions associated to names \C{f2} and \C{f2'}, we see that these
...
Now function \C{f2} can be applied to the numbers \C{2} and \C{3}, as
in \C{f2 2 3}. This results in a term of type \C{nat}, the type of the
outputs of \C{f2} and we can even compute this value:
...
Term \C{(f2 2 3)} actually reads \C{((f2 2) 3)} and features three
sub-expressions separated by spaces. The symbol \C{f2} is the deepest
sub-expression and it is applied to \C{2} in \C{(f2 2)}. The value of
this sub-expression is a function, which can be written:

\begin{coq}{}{}
fun m => 2 + m * 2
\end{coq}
Finally, \C{(f2 2 3)} is in turn the application of the latter function to
...
Earlier in this section we defined functions like \C{f} and \C{f2}
...
\marginnote{References to listings does not work}
...
\marginnote{Change the title of the section, at least to find a
  synonym for destruction.}
...
Indeed, the \C{compute} normalization strategy includes the
normalization modulo the rewrite rules
\C{f23 true $\rightarrow$  2} and \C{f23 false $\rightarrow$  2} that
model the definition by case analysis of \C{f23}. And for any new
valid definition of a function by case analysis, similar rules are
available to the \C{compute} strategy.
...
\C{(f (S n))}. Similar notations are provided for up to four
successive applications of the \C{S} constructor.
...
\C{n} is the successor of a number, then the returned value is
...
for \C{p} is \C{4}.  In this particular example, \C{p} is not used, so that we
...
actually also be written using an ``\C{if .. then .. else ..}'' like
syntax:
...
In the definition of this function, the argument \C{n} is
matched against the pattern \C{u.+1}.  When this comparison
...
Remark that we did omit the type of the input \C{n}. Indeed, matching
\C{n} against  the \C{u.+1} pattern imposes that \C{n} has type
...
\marginnote{Waiting for input from Yves, relating pattern matching
  without recursion with the complexity of the function
  implemented. See also the marginnote at definition of equality.}
...
\marginnote{Paragraph to be rewriten by Yves.}
...
\marginnote{assia: may be rename the section?}
...
existing list of type \C{(seq A)}, the tail of the sequence.  This
...
\marginnote{assia: What about the polymorphic version (head)? As an exercise?}

\subsection{Recursion for sequences}
Because the second explicit argument of \C{cons} is an arbitrarily
sequence, it is possible to build sequences that are arbitrarily long.
Yet a sequences is always finite, as a finite pile of \C{cons}
constructors, terminated with a \C{nil}.
In this respect, the type \C{seq} of sequences is very similar the
type \C{nat} of natural numbers.  Recursion is again the solution to
cope with the arbitrary
size of sequences using functions that can be described concisely.
\marginnote{assia: here again I am uneasy with this sentence. Can we
...
explicitly only one argument, \C{tl}. Indeed, remember that
%we started the
...
The type \C{pair} has two polymorphic parameters, \C{A} and
\C{B}, so
...
component and of type \C{bool} in its second, but we can also form
\C{(pair bool nat)}, \C{(pair bool bool)}, etc. he type \C{pair} is
...
value as the second one. This type inductive type has
...
\C{(a, b)} has type \C{(@pair A B)} . T For a given pair, we can
...
true : bool
\end{coqout}
As one expects pairs can be nested. \Coq{} has a slightly more complex
notation for pairs, which makes possible to write \C{(3,true,4)} for
\C{((3,true),4)}. In this example, the value \C{true} is the third
...
such a the notation for the type of pairs, the expression \C{(a * b)}
becomes ``ambiguous'',
...
in pattern matching, as the ``case analysis'' has a single
branch. There is hence a specific syntax for this artificial case
analysis on an irrefutable pattern. The above definition of projection
\C{x} can alternatively be written:
...
an input \C{x} (or \C{x'}, or \C{x1},...) its type is supposed to be
\C{T}. See \cite{Coq:manual} for more information.
...
the argument for \C{T} receives a name, so that it can be mentionned
...
not use in a given definition are omitted. For example \C{f} plays no
role in the type of \C{iter}.
...
As we mentionned in Section~\ref{sec:functions}, the \C{Eval compute}
...
\C{foldr}  under this local context gives the following result:
...
a list of \C{T}. Now if we ask for the result of some computation,
then \C{T}, \C{A} and, most importantly, \C{f} are inert symbols.
In particular, the symbol \C{f} represents an unknown, arbitray
function. Symbolic computation nonetheless normalizes expressions
featuring \C{f}, like in:
...
In order to compute expression \C{foldr init [:: x1; x2; x3]},
...
elements of type \C{nat}. We rephrase it using the \C{if .. is
  .. then} syntax presented in Section~\ref{}:
...
\C{addn} and \C{addn_alt}: the \C{addn_alt} variant transfers the
...
On the other hand,
\C{addn_alt} does not expose a successor when a recursive call is
...
subexpression featuring variable \C{i}, the one used for the
...
We already mentioned several times the \C{Notation} command, relying on the
...
Note that the definition of \C{leq} relies on the fact that
...
\marginnote{assia: this is probably hard to understand: we do not know
(yet) about modules and name spaces. I think we could include a
paragraph on name spaces here.}
...
\marginnote{assia: mention the prelude?}
...
\end{Exercise}
\begin{Exercise}[label=ex:triple,difficulty=0,title={The pair data type}]
