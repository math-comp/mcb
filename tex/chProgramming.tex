...
multiplication, comparison, are already provided for direct use.  In
...
anymore.  On the other hand, notations for addition (\C{+}),
subtraction (\C{-}) and the equality test (\C{==}) are provided by the
preamble.
...
but defines it as an alias for pre-existing type \C{list} given in the
...
existing list of type \C{list A}.

In principle, all elements of the datatype definition have a type
...
people need to give the type argument to the \C{cons} function.  This
...
coming from the fact it was used in place where a list of type \C{list
  nat} was expected.  This example also shows that the ssreflect
...
the whole list, producing a \C{.+1} construct for each
...
Notation "[ 'seq' E | i <- s ]" := (my_map (fun i => E) s)
  (at level 0, E at level 99, i ident,
   format "[ '[hv' 'seq' E '/ ' | i <- s  ] ']'").
\end{coq}
For instance, this notation makes it possible to compute successors of
a given list of natural numbers.
