...
\C{nat} annotates the output of the function with type \C{nat} --
that is, the type of the application of \C{f} to any argument will be
\C{nat}.
% Not sure if I correctly understood this sentence.
\index[concept]{type}
...
% [DG] *Three* sub-expressions? Not five? (g), (2), (3), (g 2) and
% ((g 2) 3). Maybe you mean "three nested sub-expressions".
...
% which could lead to ill-formed expressions.
% [DG] Deleted because red herring (IMHO): Ambiguities might also *not*
% lead to ill-formed expressions; these are the scarier ambiguities.
...
% [DG] Maybe a brief definition of "ill-formed" could be useful here?
% Naively I'd associate this word with "parentheses cannot be matched"
% as opposed to "types cannot be inferred", but it is the latter
% you seem to mean.
...
% [DG] Again, three *nested* sub-expressions.
...
Definition twoVthree b := if b then 2 else 3.
\end{coq}
...
\begin{coq}{name=eval_repeat_twice_f_two}{width=7cm,title=Evaluating twoVthree}
...
\C{negb}, \C{orb}, \C{andb}, and \C{implyb}, with abbreviated notations
...
\begin{coq}{name=compute-match}{width=7cm,title=Queries}
Eval compute in predn 5
\end{coq}
\begin{coqout}{}{width=5cm,title=Response}
= 4 : nat
\end{coqout}
...
construct.
% [DG] I would find the above paragraph rather confusing if I didn't
% already know its meaning.
% Actually I don't understand what "binds a subterm of
% the input" means here. Maybe better to say that on the right hand
% sides, the function being defined is only applied to "structurally
% smaller" arguments, in the sense that if we are matching our n
% against p.+1.+1, then we can only apply it to p.+1 and to p?
...
comparison (\C{leq}, \C{<=}) on natural numbers.
(As explained above, subtraction is made to return \(0\) when the
subtrahend exceeds the minuend; similarly, division is integer
division.)  The trailing \C{n} in
...
in the current setting such function has to be written twice.
...
\noindent Even if this may look like familiar jargon to some readers,
we really want this definition to be a native citizen of the
\Coq{} language, rather than a ``macro'' that would have to be
converted into a valid definition for each specific choice of \(\alpha\)
...
\C{(seq bool)} is the type of sequences of booleans. (The word
``sequence'' is synonymous to ``list''; thus, infinite sequences
are not allowed.)
...
to the type argument of \C{nil} in the built-inversion of \C{seq}
...
constructor with \C{@} and pass all arguments explicitly, as in
...
respectively. In the case when they come from a record definition,
...
and we ask \Coq{} to compute such
expression.
...
% [DG] The previous sentence is really unclear at this point.
...
The definition behind a notation can involve more than one symbol,
...
% [DG] What is the "more than one symbol" here? Maybe you wanted to
% give an example like "a is congruent to b mod c"?

\gotcha{There is no function testing if a natural number is strictly
smaller than another one.  \C{(a < b)} is just syntactic
sugar for \C{(a.+1 <= b)}}

% [DG] "syntactic sugar" should perhaps be explained.

The converse relation \C{(n > m)} is defined as a notation for
\C{(m < n)}; it is only accepted in input,
...
\item Postfix notations begin with \C{.}, as in \C{.+1} and \C{.-group}
	to let one write \C{(p.-group G)}. There is one exception for
        ...
        \C{`}, as in \C{m`}\C{!}.
        % [DG] Workaround using two \C environments, since a single
        % \C{m`!} compiles to an "m" followed by an inverted
        % exclamation mark.
...
$~$
     ...
     = false : bool
\end{coqout}
% [DG] I get the "Error: A left-recursive notation must have an explicit level."
% error when I define the "p .3" notation. I need to suffix it by
% "(at level 5)". Probably the other two analogous notations already
% have a level assigned courtesy of the mathcomp library?
...
Definition all_words n T (alphabet : seq T) :=
  ...
  iter n extend [:: [::] ].
\end{coq}
% [DG] For what it's worth, here is my solution:
