...
\end{coq}
which asserts firstly that both arguments \C{n} and \C{m} have type
\C{nat}, by the \C{(n m : nat)} type annotation, and secondly that the
...
\C{if k.+1 is p.+1 then t1 else t2} into \C{t1} where occurrences of
...
\C{predn} has type \C{nat -> nat}, an element \emph{in type} \C{nat} has to be
...
somehow added on top of the \Coq{} language. %  We want to
% reason and
...
actually write the sequence as  \C{(cons _ 2 (nil _))}, using \C{_} to
