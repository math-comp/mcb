...
applied to the natural number 2.
...
require describing in more detail the formalism underlying the
...
Observe that in the definitions of functions \C{pred} and
...
\C{subn} is to see it as a subtraction operation on natural numbers,
...
Second, starting from the next chapter we will prove properties of programs,
...
a list
or the theorems we may prove about these functions.  Hence, one might
...
Types again avoid confusion: it is not valid to form the type \C{(seq 3)},
...
to iterate constants other than \C{cons}.
...
expressions in more readable forms whenever they contain variables. Here again
...
space around the infix operator, so that notations do not get mixed up with
...
Another frequently used form of notation is called a syntactic abbreviation.
...
\item Postfix notations begin with \C{.}, as in \C{.+1} and \C{.-group}
	to let one write \C{(p.-group G)}. There is one exception for
	       a postfix notation of the factorial, which starts with
        \D{`}, as in \D{m`!}.
