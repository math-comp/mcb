...
It is worth spending a few more words on the \C{(repeat_twice f)} subexpression.
The \C{repeat_twice} program takes two arguments in input. However one
...
is passed, \C{(f : |==nat -> nat==|)}, we obtain a term which type is
the right hand side of the main arrow, that is \C{(|--nat--| -> nat)}.
By passing another argument, like \C{(2 : |--nat--|)}, we obtain an expression
which type is, again, the right hand side of the main arrow, here \C{nat}.
...
are the only way to construct a \C{bool}.
...
type to \C{nat}.  Indeed the \C{S} constructor belongs \emph{exactly}
...
for the \C{nat} datatype (hence the trailing \C{n} is omitted) and
that we will use many times in this book.  In particular the
test identifying \C{prime} and \C{odd} numbers.
...
The relevant files for this chapters are \lib{ssrbool}, \lib{ssrnat}
and, later, \lib{seq}.
...
\end{coq}
\index[coq]{\C{leq}}
Given that subtraction computes to \C{0} whenever \C{m} is less or equal
that \C{n}, such definition is correct.  Finally note that the strict order
relation is just a notation, and that \C{(m > n)} is only accepted in input
and is always printed as  \C{(n < m)}.
...
\C{(seq A)} for each possible choice of a type \C{A}.  For example
...
Here \C{Type} is a keyword that denotes the type of all datatypes.
For example \C{nat} and \C{bool} are of type \C{Type}.

Thus, \C{seq} is a function of type \C{(Type -> Type)}.
Types again avoid confusion: it is licit to form the type
\C{(seq nat)} but not \C{(seq 3)}, since \C{3} is not a datatype given
its type is not \C{Type}.
...
explicitly only one argument, \C{s'}.  This is because
%we started the
...
Note that the ambiguity between \C{(1 * 2)} and \C{(nat * bool)} can be
resolved by annotating the expression with the interpretation scope
desired: \C{(a * b)\%N} interprets the infix \C{*} as multiplication,
while \C{(a * b)\%type} would interpret \C{*} as the pair datatype
constructor.
...
Computation involving \C{f} become hence symbolic:
...
\C{foldr} takes in input a \emph{functional argument that
...
provide a generic theory of iterated operations~\ref{sec:bigop}.  Such theory becomes richer
...
\item A symbol starts with a non alpha-numerical character followed by
	any character.  For example \C{.+1} is a single symbol, as
	\C{.+4} and \C{\%/} and \C{<=} and \C{[::}.  Only existing
	symbols are taken into account: \C{3.+1.+1} is parsed as a number
	forllowed by two occurrences of the \C{.+1} symbol; \C{.+1.+1} could
...
\item postfix notataions begin with \C{.}, as in \C{.+1} and \C{.-group}
...
the \C{cons} and \C{nil} constructors the writing \C{(cons 3 nil)} 
shorthand for \C{(cons _ 3 (nil _))}.  The \C{_} denotes a term,
here a type, to be inferrer by \Coq{}.  In this case the typing constraints
of impose both \C{_} to be \C{nat}.  To locally disable the
