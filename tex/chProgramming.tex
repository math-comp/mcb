...
As we detail in Section~\ref{sec:ttch} types play an important role in \Coq{}.
In particular they are used to avoid confusion.  For example
the function \C{f} we just defined can be applied to a natural number,
i.e. a term of type \C{nat}, but not to, say, a function.
...
functions.  As a consequence it is legit to apply them partially
...
It is worth detailing the definition of \C{leq}. 
...
and is alsways printed as  \C{(n < m)}.
...
argument.  Thus, \C{seq} is a function of type \C{Type -> Type} and
...
the scope of this book, the interested reader shall refer to~\cite{Coq:refman}.
