...
\begin{coq}{}{}
Definition has_prop  T (a : T -> bool) (s : seq T) :=
   exists x0 : T, exists i, i < size s /\ a (nth x0 s i)
\end{coq}
...
A first improvement on this naive approach consists in
using an ad hoc inductive definition that resembles a
...
The trivial reflection view is called \C{idP} and is often used
...
The full proof of \C{leq_max} has been discussed in detail in section~\ref{sec:leqmax}.
...
In this section, we will further explore the possibility to improve
the flow of formal proofs by refining the shape of the definitions
specifying a given concept.
...
For instance, let us recall the definition of the \C{reflect}
predicate proposed in Section~\ref{ssec:boolProp}.
...
When making use of this predicate in a proof, it is
often the case that substituting \C{b} for its value in each branch of
the case is needed right after the case analysis.

The following alternative formulation makes the equation implicitly
stated and also automatically substituted during case analysis.
...
that states \C{(forall a b, reflect (a /\\ b) (a && b))}. The
definition of the \C{andP} view is accompanied by the following configuration
of its arguments:
...
Note that we have not specified a value for the variables quantified
in the statement of \C{andP}: the tuning of implicit arguments is key
to making lemmas easy to use, even more so in the case of the ``spec''
ones.  Thanks to the index of the \C{reflect} inductive, the automatic
substitution trivializes the second goal.  The first one can be solved
by replacing both \C{a} and \C{b} by their truth values, once \C{ab}
is destructed.  Hence the following proof script:
...
Note that if one needs to override the pattern inferred for the index
...
like an additional hypothesis in each subgoal.
...
Sometimes a proof works best by splitting into three branches, i.e.,
...
\C{(forall m, m <= n -> G m)} then one will have access to the
induction hypothesis on all numbers smaller than \C{n} exactly as
if one was using the strong induction principle.
...
without resorting to occurrence selection, which can be tricky and fragile.
...
\C{m} by \C{(0 * d.+1 + m)}.  Recall that the case \C{d} being \C{0} has
...
Note that the case analysis on \C{(ubnPgeq m)} needs to grab also
the occurrence of \C{m} in the body of \C{ed}. To do so
we push \C{ed} on the goal stack prior the case analysis.  The \C{@} modifier
tells \mcbSSR{} to keep the body of the let-in, which would be otherwise
deleted.
...
We are left with this goal to prove:
...
Line 7 prepares the goal to accommodate the application of the induction
hypothesis, instantiated at \C{q.+1} and \C{(n - d)}.

\begin{coqout}{}{}
(ed.2 < d.+1) && (q.+1 * d.+1 + (n - d) == ed.1 * d.+1 + ed.2)
\end{coqout}

Finally, line 8 uses the induction hypothesis and proves its premise
\C{(m - d <= n)}.
...
The reader already familiar with the concept of coercion may find the
presentation of this chapter slightly nonstandard.  Coercions are
usually presented as a device to model a flavor of subtyping in a type
theory that, like the \mcbCIC{}, does not enjoy such a feature.  But
as will become clear in Chapter~\ref{ch:hierarchy}, the role played by
coercions in the modeling of the hierarchy of algebraic structures is
actually minor.  In fact, it is limited to the easy task of
forgetting some fields of a structure to obtain a simpler one. The
trickiest part of setting up a hierarchy is actually the
reconstruction of the missing fields of a richer structure from a
weaker one, and the comparison of two structures in order to find the
minimum super structure.  These tasks are mainly implemented by
programming type inference, which is the main topic of the next part
of the book.
