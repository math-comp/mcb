\Chapter{Boolean reflection}{Towards real proofs}
\label{ch:boolrefl}
...
since, as schetched in section~\ref{sec:EM}, excluded middle holds in
...
Lemma implyP b1 b2 : reflect (b1 -> b2) (b1 ==> b2).
Proof.
...
case analysis of the truth table of the boolean formula.  The
``\C{;[ t1 | t2 .. | tn ]}'' syntax lets one specify a different
tactic for each subgoal (see~\cite[section 9.2]{Coq:manual}).
\index[ssr]{\C{;[..|..]}}

More
generally, a theorem stating an equivalence between a boolean
expression and a \C{Prop} statement is called a \emph{view},
since it is typically used to view an assumption from a different
perspective. 

\mantra{View names always end with a capital \C{P}.}
...
\end{coq}
which reduces to \C{((n <= m) /\\ (m <= k))} since \} is \C{true}
...
The are many ways to express the same concept, but, unsurprisingly,
one can be easier to access.  In particular there are classes of
...
Tuning of implicit arguments is key to obtain easy to use
lemmas.
...
we take $m! + 1$, then such prime divisor $p$ can be shown to be grater than $m$ as
...
(m == n) || (m < n) -> m.+1 %| (n.+1)`!	
\end{coqout}
The case analysis rules out the case \C{(m = 0)}, and simplifies the hypothesis
...
The capital \C{V} letter in the lemma name signals a disjunction.
...
i.e. \D{(n.+1 * n`!)}.  The former amounts to show that
\D{(n.+1 \%| n.+1 * n`!)}, while the latter to show that \D{(m.+1 \%| n.+1 *
...
Still it gives us the occasion to explain the \C{have} tactic, that let us
...
Shrinking proof scripts is a never ending game.  The avid reader can
see in the next section
how intro patterns let one squeeze the last two lines into a single
...
when \C{max} is computed away thanks to the extra \C{(n2 <= n1)} assumption.
...
Indeed we name the fact \C{le_mn2} just to apply transitivity to it.
...
Rewriting the top assumption fixes \C{b} to \C{n1}, trivializes 
...
The statement of the example is processed by \Coq{}
...
This notational trick is reminescent of Kronecker's $\delta$ notation.
...
of algrabraic structure is minor.  Indeed the job of
coercions in that context is to
forget some fields of a structure to obtain a simpler one, and
that is easy.  What
is hard is to reconstruct the missing fields of a structure
or compare two structures finding the minimum super structure.
These tasks are mainly implemented programming type inference.
