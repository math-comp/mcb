\Chapter{Boolean reflection}{Formalization techniques and real proofs}
\label{ch:boolrefl}
...
variables quantified in the statement of \C{andP}.  Indeed
...
It is worth mentioning the convenience view \C{boolP} that takes a boolean
formula and reasons by excluded middle providing some extra comfort
like an additional hypothesis in each sub goal.

Another view worth mentioning is \C{leqP} that replaces, in one shot,
...
separating the equality case. The \C{ltngtP} view is designed for that.
...
We prove that for any natural $m$, there exists a prime greater than $m$.
We state it using a ``synonym'' of the exists quantifier that is specialized
to carry two properties.  This way the statement is simpler to destruct.

The proof goes like that: every natural number greater than 1 has at least one
prime divisor.  If we take $m! + 1$, then such divisor $p$ can be shown to
be grater than $m$ as follows.  By contra position we assume
$p \leq m$ and we show $p \nmid m!+1$.  Being smaller than $m$,
$p | m!$, hence to divide $m!+1$, $p$ should divide $1$, that is not
possible since $p$ is prime, hence greater than 1. \hfill$\square$
...
(m == n) || (m < n) -> m.+1 %| (n.+1)`!	
\end{coqout}
The case analysis rules out the case \C{m} zero, and simplifies the hypothesis
to \C{(m < n)}.  Note that \C{(x <= y <= z)} is a notation for \C{((x <= y) &&
(y <= z))}, hence when the first inequality evaluates to true (e.g. when \C{x}
is 0, then the conjunction simplifies to the second conjunct.

If we reason by cases on the top assumption we face two easy goals, both
easy to solve if we look at the development of the factorial, i.e.
\C{(n.+1 * n`!)}.  The former amounts to show what
\C{(n.+1 \%| n.+1 * n`!)}, the latter that \C{(m.+1 \%| n.+1 * n`!)}
under the (inductive) hypothesis that \C{(m.+1 \%| n`!)}.

We can now prove the main result.  The first step is to prove that
$m! + 1$, a triviality.  Still it gives us the occasion to explain the
\C{have} tactic, that let us augment the proof context with a new fact,
typically an intermediate step of out proof.
...
The next step would be to use the \C{pdivP} lemma, maybe as a view,
and destruct its result.
...
We could hence end up with the following script.
...
It is rather unsatisfactory.  In our proof, what plays an
...
actually be an introduction pattern, i.e. what typically follows
the \C{=>} operator.  At the light of that, the script can be
...
The worst thing one can do when formalizing such an argument is to use
the worst invention of computer science: copy-paste.  The language of
...
The proof uses the following lemmas.  Pay attention to
\C{orb_idr}.
...
rewriting with \C{maxn_idPl} (as we did in Section~\ref{sec:tacview}).
Then \C{orb_idr} trivializes the main goal and generates a side condition with
an extra hyp we name \C{le_mn2}
...
symmetry step step we can resort to the \C{have} tactic. 
...
The proof for the \C{th_sym} sub proof is the text we used to copy paste.
...
the proof putting in fron the justifyication of the symmetry, and
...
Qed.
\end{coq}
The \C{suff} tactic is like \C{have} but swaps the two goals.  Note
that here the sub proof is now the shortest block.
...
can resort to the \C{wlog} tactic.
...
To keep the script similar to the previous one, we named explicilty
\C{th_sym}.  This is rarely the case in proof scripts of the library.
...
Ineed we name the fact \C{le_mn2} just to apply transitivity to it.
...
we use it to transform the top assumption \C{(m <= n2)}.  Note that
...
Rewriting it fixes \C{b} to \C{n1}, trivialized the goal \C{(m <= n1)}
to \C{true} and opens a trivial side condition \C{(n2 <= n1)}.

This very compact idiom is quite frequent in the \mcbMC{} library.

%%%%%%%%%%%%%%%5
\mcbLEVEL{2}
\mcbsubsection{Euclidean division, simple and correct}
...
The first line uses the remembering feature of the \C{:} operator
to generate an equation for the specified value (see~\cite[Section
5.5]{ssrman}) obtaining the following status:
...
E : edivn m d = ed
========================
((0 < d) ==> (ed.2 < d)) && (m == ed.1 * d + ed.2)
\end{coqout}
Line 2 handles the case of \C{d} being zero.  The ``\C{in E...}'' suffix
...
the hypotheses explicitely selected.

Line 3 prepares the induction by unfolding the definition of
...
\marginnote{if-is not printed correctly.}

\begin{coqout}{}{}
ed : nat * nat
d, n : nat
IHn : forall m n0 : nat, m <= n -> edivn_rec d m n0 = ed ->
        (ed.2 < d.+1) && (n0 * d.+1 + m == ed.1 * d.+1 + ed.2)
m, q : nat
le_mn : m < n.+1
========================
if m.+1 - d is m'.+1 then edivn_rec d m' q.+1 else (q, m.+1) = ed ->
  (ed.2 < d.+1) && (q * d.+1 + m.+1 == ed.1 * d.+1 + ed.2)	
\end{coqout}
Remark that if \C{m} is zero, the base case, the conclusion is trivial
by computation.

Line 5 uses the following lemma to push the subtraction into the
branched of the if statement, then reasons by cases on the if condition.
...
the division algorithm and is trivially solve in line 6.
...
line 7 shows that \C{(m - d <= n)}.  Line 8 concludes using
...
\marginnote{ERROR: no eqType yet}
