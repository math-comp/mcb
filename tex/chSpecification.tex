...
i.e. \C{(n.+1 * n`!)}.  The former amounts to show that
\C{(n.+1 \%| n.+1 * n`!)}, while the latter to show that \C{(m.+1 \%| n.+1 *
n`!)} under the (inductive) hypothesis that \C{(m.+1 \%| n`!)}.
...
\C{m`!.+1}.  We end up with the following, rather unsatisfactory, script.
...
Qed.
\end{coq}
The \C{suff} tactic is like \C{have} but swaps the two goals.  
...
uses the \C{/(_ ...)} intro patter to specialize the top of the stack.
...
This very compact idiom \C{/leq_trans->} is quite frequent in the
\mcbMC{} library.
