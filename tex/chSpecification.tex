...
As we have seen in \ref{ssec:indreason} the strong induction principle can be
obtained from the regular one by choosing an appropriate value for
the predicate. The \C{elim} tactic infers that predicate from the goal,
so another way to specify the predicate is to \emph{load} the goal
...
induction hypothesis on all numbers smaller than n exactly as
if he was using the strong induction principle.

The \mcbMC{} library provides a few tools to load the goal like this.

\begin{coq}{}{}
Inductive ubn_leq_spec m : nat -> Type := UbnLeq n of m <= n : ubn_leq_spec m n.
...
Real example in the following section
