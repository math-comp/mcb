...
and redesign.  For example around 2009 the way algebraic structures
are represented changed substantially with a major impact on most
files. Similarly around 2011 the \mcbSSR{} proof language evolved
substantially and the entire library was ported to the new features.

In these occasions breakage happened. And breakage and will happen
again, for many reasons, some of which are not under the control of
the author of formal proofs. For example \Coq{} is an actively
developed research software, changes in behavior, for the better, are
hard to avoid and do happen.  And even when the system does not
change, proofs may break too: Nobody gets his definitions perfect at
the first attempt in a formalization. Definitions and lemmas get
improved over time.  As a conclusion having to repair and adapt proof
scripts is an activity one can hardly avoid.
...
way in which the formal proofs were written.
...
readable, there is no questioning that: How can a proof be maintained if
...
glance, one can hardly see the entire proof (let's call these
\emph{vertical proofs}).  It is a bit like
...
most important observation drawn from horizontal proofs, is that it is
not the ``atomicity'' of steps that makes the proof readable, but
rather the fact that goals, that is statements, are more readable than
proof steps. \\
Conversely, an expert \mcbSSR{} user is typically capable of squeezing
into a single line so many steps that nobody could understand that
one-liner, even if it requires no page flipping (these could be
called \emph{horizontal proofs}).  The game of ``compressing proofs''
...
large. Horizontal proofs are indeed shorter, hence handier to
manipulate, but their poor readability is not helping maintenance.\\
The funny thing is that the tension between horizontal and vertical
...
can mix readable-but-lengthy and obscure-but-compact \emph{steps}.
...
\C{have} tactic. Such as step is as readable as the statements of
...
the statement is part of the proof text, such a step need not
the assistance of \Coq{} in order to be understood by a human.\\
A very related source of unreadability
is the choice of meaningless names for theorems or
hypotheses, e.g. \C{H7}. By asking \Coq{} to display the goal one can see
...
matrix, while using \C{x7} for any of these is not going to help
the reader what a statement is about.
...
goal''. Obviously, without seeing the actual goal, one cannot
...
machine is directed is more direct and precise.  Also, and more
...
Then, when filling the blocks, have one line of formal text perform one
meaningful proof step for a human reader, and use the best tools you have to
achieve that: at worse the reader will ignore the text, and just see what
happens. In other words giving a pace at which a proof should be read is yet
another ingredient to obtain readable proofs.

\subsection{Fail early and locally to ease repair}

There is some antagonism between being robust in face of changes and
begin easy to repair.  The former is surely tantalizing and can even
work at some extent, but it can't possibly always work. The more
sophisticated ``robustness'' is, the harder it may be to identify the
root cause of the problem (ever heard the word heuristics?).  On the
contrary the failure of a predictable, dumb, tool is likely to be
easier to understand, if only because it is simpler and less
sophisticated. The bet of the \mcbSSR{} proof language is that
predictable failure leads to easy-to-repair proofs.

Indeed, most \mcbSSR{} tactics fail early and locally. If they cannot
...
level of lemmas. For example by writing \C{have refl x : x = x} one
...
Sometimes the phenomenon is even worse. Not only the proof is large,
but also the statement is of a notable size. For example in
Aschbacher, section 23, extremal groups (such as the dihedral groups) 
are characterized by ``dictionaries'' of properties. The conjunction
of dozens of properties is proved in a single lemma, since these
properties share a great deal of their proofs. In this case the
language of intro patterns lets one decompose on the fly the
dictionary in order to access the property of interest, for example:
\begin{coq}{}{}
have [_ _ _ [_ _ maxG]] := dihedral2_structure n_gt1 genG isoG
\end{coq}
extracts the line 9 out of
\begin{coq}{}{}
...
\end{coq}
\marginpar{maybe use the latex typesetting of this, and not the coq
code (should be the same)}

\subsection{Qed is not the end of the story}

When a proof is accepted by \Coq{} it means it is correct.
It is s mistake to think the proof is OK, and in particular that it
is readable or maintainable. As we argued so far that proof is going
to eventually break, and repairing a tidy proof is much simpler than
repairing a messy one.
...
Unfortunately little support is given to the user to tidy proofs up by
today's UIs, even for very recurrent steps such as avoid duplication
within the current proof or across the library, that is figuring out
that some existing lemmas were re-proved inline (because one did not
know they are already part of the library). As one can expect, avoid
duplication is key to lower maintenance effort.
