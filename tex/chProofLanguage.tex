...
to denote silly steps, e.g. \C{addnCA} to shuffle summands around.
What is left is the salient contents of the obscure step.

How much of that is tied to the technology we have today?
...
because \C{lem1}'s side condition get's changed and \C{lem2} is not
helping anymore and \C{//} cannot solve it, then
\C{lem3} is likely to fail, since it is meant to apply to the main
...
following line (after \C{lem1} all actions never fail).
...
it with a stronger one. For example to refine \C{H} one
can write \C{have \{H\}H : refined_ty}.
...
Unfortunately there is no free lunch: when a proof is accepted by
