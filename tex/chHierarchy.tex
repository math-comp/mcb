...
While using amd populating a structure hierarchy is fairly
...
\lstinline/(operator $\mcbimplm{L}$)$\simeq$andb/, by setting
\lstinline/$\mcbimplm{L}\equiv$andb_monoid/.
...
spurrious ``Projection value'' warnings; this is also done in the actual
...
sequences of \C{choiceType}s are also \C{choiceType}s.
...
\emph{element} of a \C{T : choiceType} as an element of an \C{eqType},
...
\gotcha{If you see an undocumented \lstinline/GRing.toto/,
