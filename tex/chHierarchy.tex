...
\mcbPROVIDE{\C{fintype}, \C{GRing}, \C{zmodp}}
\mcbREQUIRE{\C{eqType}}
...
documentation of the \C{ssralg} library, which provides structures for
...
of functions for creating instances. For structures like \C{eqType}
...
\item \C{EqType}: an instance construtor that creates an instance of
...
examples from the \C{zmodp} library that puts a ``mod $p$'' algebraic
structure on the type \C{ordinal p} of integers less than $p$.
The \C{ordinal} type is defined in library \C{fintype} as follows:
...
Algebra only makes sense on non-empty types, so \C{zmodp} only defines
...
into \C{'I_p} by applying the modolus.  Indeed the
...
Creating an instance of the lowest \C{ssralg} structure, the
...
\C{ssralg} becoms applicable to \lstinline/'I_p/; for example \C{0}
...
instance will be used. This can be important (though not in this case)
as unfolding definitions can affect canonical structure inference.

Line 2 does however perform some nontrivial inference, because
although \C{zmodType} is the first \C{ssralg} structure, it is not at
...
create a full \C{zmodType} instance --- section \ref{sec:phant} gives
the technical details.

Rings are the next step in the algebraic hierarchy. In order to
simplify the theory of polynomials \C{ssralg} only provides structures
...
Line 7 endows \lstinline/'I_p/ with a \C{ringType} structure, making
it possible to multiply in \lstinline/'I_p/ or have polynomials over
\lstinline/'I_p/. Line 8 adds a \C{comRingType} commutative ring
...
avoid the unpleaseantness of multiple interpretations of \C{0}
...
modulus is an abstract constant (say, \lstinline/Variable p/), and \C{zmodp}
...
While \C{ssralg} and \C{zmodp} provide the instances type inference
...
type, and the \C{fintype} library declares a corresponding \C{finType}
...
constructor requires only one line in \C{zmodp}.
...
telescopes were not suitable for the main \C{Type} structure hierarchy
of the \mcbMC{} library, including \C{eqtype}, \C{choice}, \C{fintype}
and \C{ssralg}. However, as we have seen in
...
of a teelscope hierarchy. Recall that \C{Monoid.law} associates an
...
to just \C{true},\footnote{There is no spurrious
...
\lstinline/com_operator/ argument $\mcbimplm{L}$ of \C{ComLaw} by
...
\lstinline/$\mcbimplm{L}=$andb_monoid/ as above.
...
choice, which is derivable in \mcbCIC{}. The stronger requirement
...
is detailed in the comments of \C{choice.v}.
...
the \C{ssralg} library is \C{zmodType}$=$\C{GRing.Zmodule.type}; it
...
Similarly to the definition of \lstinline/eq_op/ in \C{eqtype}, the
...
\C{ssralg}, and also given the usual arithmetic syntax (\C{0},
...
x ^n / y^n$ that hold only for such rings, \C{ssralg} provides a
...
Lines 1--6 below \C{comUnitRingType} to each of the 6 structures it
...
\C{comRingType} and \C{unitRingType} in the structure hierarchy.
...
explicit ``join'' links to a telescope hierarchy (\C{ssralg} does this
for its algebraic predicate hierarchy).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\mcbLEVEL{3}
\mcbLEARN{Hiding infered structures in parameters and constructors}
...
\end{itemize}
The first case occurs in \C{ssralg} for structure types for modules
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Linking a custom datatype to the library}

The SubType kit of Chapter~\ref{ch:sigmabool} is not the only
...
\end{coq}
Then one can show that they cancel out.

\begin{coq}{}{}
Lemma w_is_ord4 : pcancel w2o o2w.
...
\begin{coq}{}{}
Definition windrose_eqMixin := PcanEqMixin w_is_ord4.
Canonical windrose_eqType := EqType windrose windrose_eqMixin.
Definition windrose_choiceMixin := PcanChoiceMixin w_is_ord4.
Canonical windrose_choiceType := ChoiceType windrose windrose_choiceMixin.
Definition windrose_countMixin := PcanCountMixin w_is_ord4.
Canonical windrose_countType := CountType windrose windrose_countMixin.
Definition windrose_finMixin := PcanFinMixin w_is_ord4.
Canonical windrose_finType := FinType windrose windrose_finMixin.	
\end{coq}
Only one tiny details is left on the side.  To use \C{windrose}
...
More in general a datatype \C{T} can be equipped with a
eqType, choiceType, and countType structure by
