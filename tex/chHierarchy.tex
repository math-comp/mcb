...
like the one at line 4 is needed to define \C{Zp_comRingType}:
...
strategy, the \C{find} operator, for searching and electing a
...
defined using the \C{find} operator provided by the interface, and its properties follow from the axiom of countable choice, which is derivable in \mcbCIC{}% \footnote{behind the
...
\C{choiceType}s, so that pairs or sequences of \C{choiceType}s are
also \C{choiceType}s.  This subtlety is detailed in the comments of
the \lib{choice} file.
...
the other hand, postulating that any type is endowed with a
structure of \C{choiceType} is a consistent extension to the Calculus
