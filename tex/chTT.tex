...
mathematical statements and proofs, called \emph{Gallina}. The
language is a dialect of the type theory called the \emph{Calculus of
  Inductive Constructions} (CIC)~\cite{coquand:huet:88,CoPa89}, a
descendent of so-called Martin-Löf intentionnal type theory. In this
section, we provide some hints on the main features of this
formalism. Gallina is described formally in chapter 4
of the reference manual of \Coq{}~\cite{Coq:manual}. The first
chapter of the reference book on homotopy type theory~\cite{hottbook},
as well as its annex provide a presentation of Martin-Löf
...
\emph{Type theory}, actually in several different flavors, can be used
as a foundational language for mathematics. It is thus an alternative
to the flavors of set theory usually cited as the formal language of
...
$$\Gamma \vdash t : T$$
and read:
\emph{in the context $\Gamma$, the term $t$ has the type $T$}. A
context is a list of variables, each paired with a type, that can occur
in $t$ and $T$. This typing judgement expresses that under the typing
assumptions listed in the context $\Gamma$, $t$ and $T$ are
well-formed, and moreover that term $t$ has type $T$. Example:

$$x : \mathbb{N} \vdash x + x : \mathbb{N}$$


The proof that a typing judgement is valid piles typing rules of the
type theory, up to atomic ones like:

$$x : T \vdash x : T$$


A call to the \C{Check} command introduced in Chapter~\cite{ch:prog} 
verifies that a certain typing jugment holds in the current
...
Our last exemple is a typing judgement  $\Gamma \vdash t : T$ where
the term $t$ is a proof. Indeed, in the same context, the term
...
is available in \Coq{}'s reference manual~\cite{Coq:manual}.
...
thought of as a label for a collection of terms, and the judgment $t : T$ as the
statement that ``term $t$ belongs to the collection $T$'', or 
$t\in T$. 
...
$A$. Second, a judgement is not a proposition in the same sense as the
...
and called \emph{sorts}.  Gallina has a countable collection of sorts,
but in the present book, we will only make use of sorts \C{Prop} and
\C{Type}. In an empty context, the term \C{Prop} has type \C{Type}:
...
Terms are defined starting from a countable infinite set of
...
natural numbers and one of type \C{{poly int}}, i.e. a polynomial in
$\mathbb{X}[X]$:
\begin{coq}{}{}
Variables (n m : nat) (p : {poly int}).
\end{coq}
Hypotheses are themselves variables, and use the same collection of
names. The following line assumes a hypothesis:
\begin{coq}{}{}
Hypothesis neq0 : n = 0.
\end{coq}
It is a synonym of:
