...
$t$, or to disprove (internally) that $t : T$ for some particular
...
for those on the right, i.e., for types.
...
be we will only make use of sorts \C{Prop} and
\C{Type}.\marginnote{Mind the  hierarchy.} Sorts are atomic types and
in an empty context, the term \C{Prop} has type \C{Type}:
...
symbols. These names are used to name  \emph{variables}
available for names, so that it is always possible to exhibit a name
distinct from a given arbitrary finite collection of names. In \Coq{}
syntax, we can use letters, or sequences of alpha-numeric characters
for this purpose:
