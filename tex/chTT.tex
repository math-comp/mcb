...
\coqrun{name=r1}{ssr,check-3}
 ...
 In a context containing a certain variable \C{b} with type \C{bool}, the
 term \C{\~\~ b} has type \C{bool}:
...
~~ b : bool
\end{coqout-right}
\coqrun{name=r2}{ssr,check-negb}

And \Coq{} complains if we try to verify the typing jugment asserting it
has type \C{nat}. In this case, it even computes and displays the
correct type:
...
\begin{coqout-right}
Error:
The term "negb b" has type "bool" while it is expected to have type "nat".
\end{coqout-right}
\coqrun{name=r2}{ssr,check-negb}
...
Terms are constructed via an inductive definition.
...
\emph{variables}, which are among the simplest, atomic terms.
We shall also use this collection of names for constants (for instance
to introduce definitions). Beside variables, the other atomic terms
are  called \emph{sorts} and they are denoted using a distinct
countable collection of symbols. The constants \C{Prop} and
\C{Type} encountered in the previous chapters are sorts.
