...
descendant of so-called Martin-Löf intentional type theory. In this
...
\emph{Dependent type theory}, actually in several different flavors, can be used
...
default hidden to the user.  In the following typing judgment:
...
index than the instance of \C{Type} on the left. In an empty context,
...
\begin{coq-left}{name=check-bool-nat}{}
Check nat -> bool.
\end{coq-left}
\begin{coqout-right}
bool -> nat : Type
...
When a defined function is applied to its argument, the resulting term
is \emph{computed} by substituting the bound variable for the value of the
argument.
...
respectively. This is an instance of the so-called Curry-Howard
isomorphism. Let us see this remark at work on a few simple examples.
