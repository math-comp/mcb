...
presentation, although an exhaustive description shall remain out of
...
structures than the mere functions of the formalism described in
...
arguments: a proof of \C{A} and a proof of \C{B}. This way, it
models faithfully the logical rule \C{$\wedge_I$}.  Note that \C{A}
and \C{B} are parameters of the inductive definition, similarly to the
definition of polymorphic lists or pairs.
...
\C{(ex nat (fun x : nat => (odd x) && (prime x)))}. Note like just
like type \C{seq} is polymorphic, with one type parameter, the type
of \C{ex} and \C{ex_intro} are also polymorphic and they have two
parameters \C{A} and \C{P}:
...
description of the formal proof being constructed and provides support
for taming the bookkeeping part of the proof.
