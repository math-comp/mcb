...
\marginpar{We cannot display the unset printing notations, and the
  variable disappears as well. Would like to display the output too.}
...
passed to this constructor are equal.\marginpar{Provide example/exercise}
...
function \marginpar{Put match instead?} by:
...
command expects termination to be visible from a syntactic criterium
like ``all recursive calls happen on a strict sub-term''. Indeed, allowing
non-terminating computations for well-typed terms would interact badly
with the conversion rule, and ultimately lead to proofs of absurdity
(see section~\ref{ssec:indreason}). \marginpar{And say something about
non structural?}
