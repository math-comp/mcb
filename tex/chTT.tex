...
We will use the same class of terms to represent both functions and
proofs of formal statements, themselves represented by the types of
the functions. Constructing a proof for a given formal statement
\emph{is} constructing a term of the corresponding type.
Consider the simple example of formal statement stating
that a proposition $A$ implies itself, namely $A \rightarrow A$.
Now the implication symbol~$\to$ can be understood as the symbol we used
to represent the type of functions, in the non-dependent case. There
is a simple program having
the type $A \rightarrow A$: it is the identity function for terms of
type $A$ and the term representing this function is
\C{(fun x : A => x)}. If we consider that $A$ is a
proposition, this program takes in input a term $x$ of type $A$, seen
as a proof of $A$, and produces a proof of $A$ by outputing its
argument.
