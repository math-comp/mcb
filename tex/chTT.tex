...
\begin{coqout}{name=Stack2}{width=5cm}
 ...
 ~~ odd y -> 2 < y.+1 + x
\end{coqout}
\index[ssr]{\C{=>}!\C{/view} (view application)}
\index[ssr]{\C{=>}!\C{//} (close trivial goals)}
...
\gotcha{The case intro pattern \C{[..|..]} obeys an exception: whe it is the first
item of an intro pattern, it does not perform a case analysis, but only branch
on the subgoals.\footnote{not the status quo, but I prefer this exception to the
current one} Indeed in \C{case: n => [|m]} only one case analysis is performed.}
...
\begin{coqout}{name=Stack2}{width=6cm}
 ...
 0 < x -> odd y -> 2 < y + x
\end{coqout}
\index[ssr]{\C{=>}!\C{/view/view} (multiple views)}
...
3 * 2 = 3 + 3 -> 6 = 3 + 3
\end{coqout}
\index[ssr]{\C{=>}!\C{/(_ arg)} (specialization)}
...
when one specializes the top assumption as in \C{/(_ x)} he is forming the
...
6 = 3 * 2
\end{coqout}
\index[ssr]{\C{=>}!\C{->} (rewriting L2R)}
\index[ssr]{\C{=>}!\C{<-} (rewriting R2L)}
...
\end{coqout}
\index[ssr]{\C{:}!\C{term} (generalization)}
...
========================
 odd y'.+1 -> 2 < y'.+1 + x
\end{coqout}
\index[ssr]{\C{:}!\C{E: term} (equation generation)}
...
on \C{A}.  Remark that also \C{seq} is polimorphic, but only in one argument
...
quantification on the head elemt, while here \C{a} needs to occur in
...
the dimention of the resulting matrix in terms of the size of the input
...
%           Type -> Type  a poly data type construtor
