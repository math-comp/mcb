...
The \mcbMC{} library provides a \emph{sub-type kit} to lext one
...
so that it checks that a proof that the domain is finite has been
recorded before. This is using the tricks
explained in section~\ref{sec:phantom}. This results in \Coq{}
rejecting the term \C{\{ffun nat -> nat\}} but accepting
\C{\{ffun 'I_7 -> nat\}}. In the rest of the chapter, we will use this
notation
...
Similarly to fniite functions we can declare a coercion to let
