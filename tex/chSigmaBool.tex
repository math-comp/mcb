...
An first application of the type of finite functions is the following
lemma.
...
The first application of finite functions is in the definition of
...
Similarly to finite functions we can declare a coertion to let
...
\begin{coq}{}{}
Definition perm_on T S : pred {perm T} :=
...
We will decribe such type only in the next chapter.  From now
...
determinant using Leibnitz's formula.
...
that is \C{0} if \C{s} is an even permutation, \C{1} otherise,
...
checker will not understand automatically that a suqare matrix of
...
\mcbLEVEL{1}
\mcbsubsection{Two proofs about matices}

Let's work out an example. ... Note size of B is inferred.
...
mkE unravels the body of \C{(A *m B) i i}

but we have to do the same, with same proof up to A B.
...
Proof.
gen have expand_trM, -> :
  m n A B / \tr (A *m B) = \sum_i \sum_j A i j * B j i.
  by apply: eq_bigr => i _; rewrite mxE.
...
describe gen have.  exchange big to align the indexes.
then boils down to
...
B : 'M_(n, m)
============================
\sum_(j < n) \sum_(i < m) A i j * B j i =
\sum_(i < n) \sum_(j < m) B i j * A j i
\end{coqout}

Another commented example. maybe this one is too big.
...
\mcbLEVEL{2}
\mcbsubsection{blocks and casts}\label{sec:castmx}
